...
const LEADS_DB = process.env.NOTION_LEADS_DB_ID; // 899cfc17...
...
export default async function handler(req, res) {
  ...
  try {
    ...
    // 1. Write to Notion leads database
    const properties = {
      Name: { title: [{ text: { content: name || "Unknown" } }] },
      Email: { email: email },
      "Lead Source": {
        select: { name: source || "Website - Date Checker" },
      },
    ...
    if (guests) {
      properties["Guest Count"] = {
        rich_text: [{ text: { content: guests } }],
      };
    }
